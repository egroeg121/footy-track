...
def _init_state() -> None:
    ...
    # Background warmup disabled — model is too large to hold two instances in memory.
    # JIT cache (~/.cache/torch_inductor_sam3) persists across restarts instead.
...
def main() -> None:  # noqa: PLR0912, PLR0915
    ...
    st.caption("⚠️ Disable 'Always rerun' (top-right menu) before clicking — re-runs mid-propagation will crash the session.")
    if st.button("▶️ Label video", type="primary", disabled=not objects):
        progress = st.progress(0.0, text="Loading & compiling model (first run only)…")
    ...
    if frames:
        ...
        if ok:
            ...
            st.image(preview, channels="RGB", use_container_width=True)
