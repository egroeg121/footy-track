"""Method B (ft-xps): box-prompted SAM2 image predictor on ROI.

Implements the BallTracker protocol for the bake-off harness (ft-1my).
Assembled onto the bake-off scratch branch by ft-5hd.2 from
``origin/polecat/quartz/ft-xps`` (module originally lived at
``footy_track.ball_tracking.sam2_tracker`` on that branch; moved here to
match the other bake-off methods' ``ball_trackers`` package).
...
Weights: the ``SAM(model_name)`` call downloads the Ultralytics SAM2
checkpoint (default "sam2_b.pt") on first use of ``track()`` — construction
of ``Sam2BallTracker`` itself does NOT touch the network or require weights
(model is lazy-loaded in ``_ensure_model``), so this module can be imported
and instantiated in environments without the checkpoint pre-downloaded.
See ft-5hd.3 for pre-downloading the checkpoint / unblocking a real run.
"""
...
class Sam2BallTracker:
    ...
    Bake-off method B (ft-xps). Implements BallTracker
