    ...
    with tqdm(total=total, desc="uploading", unit="img") as bar:
        ...
        def _upload_one(job):
            ...
            try:
                ...
                # SDK bug (roboflow==1.2.11, rfapi.save_annotation): a 409
                # "already annotated" response returns error as a bare string,
                # but the SDK assumes {"message": ...} and crashes reading it.
                # A 409 here means this image already has this annotation
                # attached (e.g. from an earlier/interrupted run) -- success,
                # not a failure.
