"""
Script to train a YOLO classifier model.
"""

import argparse
import os
from datetime import datetime

import torch
from roboflow import Roboflow
from ultralytics import YOLO


def train_classifier(
    model_name: str,
    dataset_version: int,
    freeze_layers: int,
    epochs: int,
    name_prepend: str = "",
):
    """
    Downloads a dataset from Roboflow and trains a YOLO classifier model.

    Args:
        model_name (str): The name of the YOLO model to use (e.g., 'yolo11n-cls').
        dataset_version (int): The version of the dataset on Roboflow.
        freeze_layers (int): The number of layers to freeze during training.
        epochs (int): The number of epochs to train for.
        name_prepend (str): A string to prepend to the run name.
    """
    # Roboflow and dataset setup
    print("Downloading dataset from Roboflow...")
    rf = Roboflow(api_key=os.environ.get("ROBOFLOW_API_KEY"))
    project = rf.workspace("egroeg121").project("footy-track-broadcast-frame")
    version = project.version(dataset_version)

    # Assuming the script is run from the root of the project directory
    dataset_location = f"data/classifier_dataset/roboflow_dataset_{dataset_version}"
    dataset = version.download(model_format="folder", location=dataset_location)

    print(f"Dataset downloaded to: {dataset.location}")

    # Check for GPU availability
    print(
        f"MPS available: {torch.backends.mps.is_available()}, MPS built: {torch.backends.mps.is_built()}"
    )

    # Set up training run
    run_name = f"{datetime.now():%Y-%m-%d_%H-%M}_model_name={str(model_name)}_dataset_version={dataset_version}_epochs={epochs}_freeze_layers={freeze_layers}"
    if name_prepend:
        run_name = f"{name_prepend}_{run_name}"
    model_path = f"{model_name}.pt"

    print(f"Starting training for run: {run_name}")
    model = YOLO(model_path)

    # Train the model
    results = model.train(
        data=dataset.location,
        epochs=epochs,
        imgsz=224,
        freeze=freeze_layers,
        cache=True,
        augment=True,
        plots=True,
        device="mps",
        project="footy_scan_classifier",
        name=run_name,
    )
    print("Training complete.")
    print(f"Model and results saved to: {results.save_dir}")
    # The metric for classification is accuracy
    return results.results_dict["metrics/accuracy_top1"], os.path.join(
        results.save_dir, "weights", "best.pt"
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a YOLO classifier model.")
    parser.add_argument(
        "--model",
        type=str,
        default="yolo11n-cls",
        help="The YOLO model to use (e.g., yolo11n-cls). '.pt' will be appended.",
    )
    parser.add_argument(
        "--dataset-version",
        type=int,
        default=10,
        help="The version of the dataset to download from Roboflow.",
    )
    parser.add_argument(
        "--freeze",
        type=int,
        default=9,
        help="Number of layers to freeze during training.",
    )
    parser.add_argument(
        "--epochs",
        type=int,
        default=50,
        help="Number of epochs to train for.",
    )

    args = parser.parse_args()

    train_classifier(
        model_name=args.model,
        dataset_version=args.dataset_version,
        freeze_layers=args.freeze,
        epochs=args.epochs,
    )
